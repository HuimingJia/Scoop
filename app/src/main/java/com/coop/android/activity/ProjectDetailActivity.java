...
public class ProjectDetailActivity extends Activity implements View.OnClickListener,DialogInterface.OnClickListener{
	private Button mEditBtn; // Edit Button
	...
	private void initiateViews() {
		...
		if (BmobUser.getCurrentUser(User.class).getObjectId().equals(mProject.getLeader().getObjectId())) {
			...
			mDelBtn.setText("Drop The Project");
			mConfDelDialog.setTitle("Confirm Drop?");
			mConfDelDialog.setMessage("Are You Sure Drop The Project?");
		}		
	}
	...
	private void initiateEvents()
	{
		...
		mConfirmDialog.setButton1("确定",this);
		mConfirmDialog.setButton2("取消",this);
		mConfDelDialog.setButton1("确认",this); 
		mConfDelDialog.setButton2("取消",this);
	}
		...
		if (v == mBackBtn) {
			...
			if (!iEditOrSave) {
				if (BmobUser.getCurrentUser(User.class).getObjectId().equals(mProject.getLeader().getObjectId())) {
					...
					CustomToast.showCustomToast("You Do Not Have Authority To Delete",this);
				}
			} else {
	...
	private void DismissProject() {
		mProject.delete(new UpdateListener() {
			...
			public void done(BmobException e) {
				if (e == null) {
					CustomToast.showCustomToast("Delete Project Finished",ProjectDetailActivity.this);
				}
			}
		});
		...
		BmobUser.getCurrentUser(User.class).update(new UpdateListener() {
			...
			public void done(BmobException e) {
				if (e == null) {
					CustomToast.showCustomToast("Delete Project Finished", ProjectDetailActivity.this);
					Intent intent = new Intent(ProjectDetailActivity.this,ScoopActivity.class);
					startActivity(intent);
			...
			public void done(List<User_Project> list, BmobException e) {
				if (e == null) {
					list.get(0).delete(new UpdateListener() {
						...
						public void done(BmobException e) {
							if (e == null) {
								CustomToast.showCustomToast("Drop Finished",ProjectDetailActivity.this);
								Intent intent = new Intent(ProjectDetailActivity.this,ScoopActivity.class);
								startActivity(intent);
								ProjectDetailActivity.this.finish();
							} else {
								CustomToast.showCustomToast("Drop Failed", ProjectDetailActivity.this);
							}
						}
					});
				} else {
					CustomToast.showCustomToast("Can Not Get RelationShip！",ProjectDetailActivity.this);
				}
			}
		});
	}
	...
	private void SaveModification() {
		...
		mProject.update(new UpdateListener() {
			...
			public void done(BmobException e) {
				if (e == null) {
					CustomToast.showCustomToast("Update Finished",ProjectDetailActivity.this);
				} else {
					CustomToast.showCustomToast("Update Failed",ProjectDetailActivity.this);
				}
			}
		});
